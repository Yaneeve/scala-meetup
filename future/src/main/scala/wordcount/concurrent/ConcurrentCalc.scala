...
object ConcurrentCalc extends App with LazyLogging {
  ...
  val eventualMapReduce: Future[Map[String, Int]] = (map1, map2).mapN(_ |+| _)
  val max: Future[(String, Int)] = eventualMapReduce.map(_.maxBy(_._2))
